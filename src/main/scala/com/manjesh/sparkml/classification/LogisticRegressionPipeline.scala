...
object LogisticRegressionPipeline {
  ...
  def logisticRegressionPipeline(features: VectorAssembler, dataFrame: DataFrame) = {
    ...
    /*holdout.rdd.map(x => x(0).asInstanceOf[Double]).repartition(1).saveAsTextFile("./results/LR.xls")
    holdout.rdd.map(x => x(1).asInstanceOf[Double]).repartition(1).saveAsTextFile("./results/Actual.xls")

    savePredictions(holdout, dataFrame, rm, "./results/LogisticRegression.csv")*/
  }
